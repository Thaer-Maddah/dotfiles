...
[[ -n $2 ]] && file=$2 || { file="."; }
# Playing video
#du -a "$file" | cut -f2- | grep -iE "$ext" | dmenu -i -l 30 -sb "$menu_color"  | xargs -I {} mpv {} "$option" >/dev/null 2>1 & disown  # Run video and diswoen mpv # || clear && dialog --msgbox "MPV does not support selected file format" 5 45'

du -a "$file" | cut -f2- | grep -iE "$ext" | dmenu -i -l 30 -sb "$menu_color"  | xargs -I {} mpv {} "$option" 
