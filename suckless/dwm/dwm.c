...
drawbar(Monitor *m)
{
    ...
    // Thaer Maddah
    // remove tw - 2
		drw_text(drw, m->ww - tw - 2 * sp, 0, tw , bh, 0, fribidi_text, 0);
	for (c = m->clients; c; c = c->next) {
		occ |= c->tags;
		if (c->isurgent)
			urg |= c->tags;
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drw_text(drw, x, 0, w * sp , bh, lrpad / 2, fribidi_text, urg & 1 << i);
	...
	x = drw_text(drw, x, 0, w, bh, lrpad / 2.5, fribidi_text, 0);
